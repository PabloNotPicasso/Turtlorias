...
{
    {
        ...
        EXPECT_EQ(v.len(), sqrt(v.x * v.x + v.y * v.y));
    }
...
{
    {
        const Vector a = Helpers::getRandom<Vector>(-1e5, 1e5);
        const Vector b = Helpers::getRandom<Vector>(-1e5, 1e5);
        EXPECT_EQ(Vector::scalar(a,b), (a.x*b.x + a.y*b.y));
    }

    {
        const Vector a = Helpers::getRandom<Vector>(-1e5, 1e5);
        const Vector b(-a.y, a.x);
        EXPECT_EQ(Vector::scalar(a,b), 0);
    }

    {
        const Vector a = Helpers::getRandom<Vector>(-1e5, 1e5);
        const Vector b(a);
        EXPECT_EQ(Vector::scalar(a,b), a.len()*b.len());
    }
}
...
{
    {
        const Vector a = Helpers::getRandom<Vector>(-1e5, 1e5);
        const Vector b = Helpers::getRandom<Vector>(-1e5, 1e5);
        EXPECT_EQ(Vector::vectorProduct(a,b), (a.x*b.y - a.y*b.x));
    }

    {
        const Vector a = Helpers::getRandom<Vector>(-1e5, 1e5);
        const Vector b(-a.y, a.x);
        EXPECT_EQ(Vector::vectorProduct(a,b), a.len()*b.len());
    }

    {
        const Vector a = Helpers::getRandom<Vector>(-1e5, 1e5);
        const Vector b(a);
        EXPECT_EQ(Vector::vectorProduct(a,b), 0);
    }
}