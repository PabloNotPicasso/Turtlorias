...
public:
    ...
    static bool areIntersected(const Segment& a, const Segment& b)
    {
        ...
        bool areOnDifferentSides = Vector::vectorProduct(aVec, Vector(a.start, b.start))
                * Vector::vectorProduct(aVec, Vector(a.start, b.end))
            < 0;
        areOnDifferentSides &= Vector::vectorProduct(bVec, Vector(b.start, a.start))
                * Vector::vectorProduct(bVec, Vector(b.start, a.end))
            < 0;

        bool belongToEachOther
            = a.belongs(b.start) || a.belongs(b.end) || b.belongs(a.start) || b.belongs(a.end);
    ...
    static bool areIntersected(const Segment& a, const Ray& ray)
    {
        ...
        bool pointsAreOnDifferentSides = Vector::vectorProduct(rayVec, Vector(ray.start, a.start))
                * Vector::vectorProduct(rayVec, Vector(ray.start, a.end))
            < 0;

        bool angleCheck
            = Vector::scalar(Vector(ray.start, ray.direction), Vector(ray.start, a.start)) > 0
